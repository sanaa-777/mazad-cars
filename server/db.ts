...
import { demoListingDetail, demoPublicListings } from "./demoListings";
...
export async function listingDetails(id: number) { const demo = demoListingDetail(id); if (demo) return demo; const db = await getDb(); if (!db) return null; await syncAuctionStatuses(); const listing = (await db.select().from(listings).where(eq(listings.id, id)).limit(1))[0]; if (!listing) return null; const [images, auction] = await Promise.all([db.select().from(listingImages).where(eq(listingImages.listingId, id)).orderBy(listingImages.sortOrder), db.select().from(auctions).where(eq(auctions.listingId, id)).limit(1)]); const bidHistory = auction[0] ? await db.select({ bid: bids, bidder: users.name }).from(bids).leftJoin(users, eq(users.id, bids.bidderId)).where(eq(bids.auctionId, auction[0].id)).orderBy(desc(bids.createdAt)).limit(20) : []; return { listing, images, auction: auction[0] ?? null, bidHistory, isDemo: false }; }
