...
export function MarketplaceLayout({ children, title, actions }: MarketplaceLayoutProps) {
  ...
  const submitHeaderSearch = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const query = headerQuery.trim(); navigate(query ? `/listings?query=${encodeURIComponent(query)}` : "/listings"); };
  return <div className="min-h-screen bg-[#f4f6f9] text-[#18243b]" dir="rtl">
    <header className="sticky top-0 z-40 border-b border-white/10 bg-[#172b4d]/95 text-white shadow-[0_8px_26px_rgba(13,30,57,.16)] backdrop-blur">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-3 px-4 sm:px-6">
        ...
        <nav className="hidden items-center gap-1 lg:flex" aria-label="التنقل الرئيسي"><Link href="/" aria-current={location === "/" ? "page" : undefined} className={`rounded-lg px-3 py-2 text-sm font-bold transition focus-visible:ring-2 focus-visible:ring-[#f7ad32] ${location === "/" ? "bg-white/15 text-white" : "text-slate-200 hover:bg-white/10"}`}>الرئيسية</Link><Link href="/auctions" aria-current={location.startsWith("/auctions") ? "page" : undefined} className={`rounded-lg px-3 py-2 text-sm font-bold transition focus-visible:ring-2 focus-visible:ring-[#f7ad32] ${location.startsWith("/auctions") ? "bg-white/15 text-white" : "text-slate-200 hover:bg-white/10"}`}>المزادات</Link><Link href="/listings" aria-current={location.startsWith("/listings") ? "page" : undefined} className={`rounded-lg px-3 py-2 text-sm font-bold transition focus-visible:ring-2 focus-visible:ring-[#f7ad32] ${location.startsWith("/listings") ? "bg-white/15 text-white" : "text-slate-200 hover:bg-white/10"}`}>الإعلانات</Link></nav><form onSubmit={submitHeaderSearch} className="relative hidden min-w-0 max-w-sm flex-1 xl:flex" role="search"><label className="sr-only" htmlFor="header-marketplace-search">ابحث في سوق السيارات</label><Search className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#f7ad32]" aria-hidden="true" /><input id="header-marketplace-search" value={headerQuery} onChange={(event) => setHeaderQuery(event.target.value)} placeholder="ابحث عن سيارة أو موديل" className="h-10 w-full rounded-xl border border-white/15 bg-white/10 pr-9 pl-10 text-sm text-white placeholder:text-slate-300 outline-none transition focus:border-[#f7ad32] focus:bg-white/15 focus:ring-2 focus:ring-[#f7ad32]/40" /><button type="submit" aria-label="تنفيذ البحث" className="absolute left-1 top-1 h-8 rounded-lg bg-[#f7ad32] px-2 text-xs font-black text-[#172b4d] transition hover:bg-[#ffc25a] focus-visible:ring-2 focus-visible:ring-white">بحث</button></form>
